-- Activa rol comercio + comercio/sucursal para un email existente.
-- Útil si el mismo correo ya era repartidor o quedó sin rol merchant.
...
DECLARE
  ...
  v_merchant_id UUID;
  v_branch_id UUID;
  v_email TEXT := lower('tutacanehuillca@gmail.com');
  v_name TEXT;
BEGIN
...
LEFT JOIN public.merchant_users mu ON mu.user_id = u.id AND mu.is_active = TRUE
LEFT JOIN public.merchants m ON m.id = mu.merchant_id
WHERE u.email = lower('tutacanehuillca@gmail.com')
GROUP BY u.email, m.id, m.name;
