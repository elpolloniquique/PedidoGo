-- PedidosGo Fase 12: seguimiento público por tracking_token
-- Acceso anónimo vía SECURITY DEFINER (sin abrir RLS de tablas a anon).
...
SELECT 'public_tracking', TRUE, 'Seguimiento público del cliente (Fase 12)'
WHERE NOT EXISTS (
  SELECT 1 FROM public.feature_flags WHERE key = 'public_tracking'
);

-- ---------------------------------------------------------------------------
-- get_public_tracking(token) — DTO seguro para customer-tracking
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_public_tracking(TEXT) IS
  'Fase 12: seguimiento público por token. No expone pickup_code ni PII del driver.';
